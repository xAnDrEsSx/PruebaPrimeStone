		...
		var defaults = {
			format_output: function(tim, mini, meri) {
			    if (settings.show_meridian) {
					...
					let momento = moment(tim + ":" + mini + " " + meri, ["h:mm A"]).format("HH:mm");
		...
		return this.each(function() {
			...
			function open_timepicki() {
				...
				if(!settings.input_writable) {
					...
					first_input = ele_next.find('input:visible').first();
					first_input.focus();
				}
			...
			function change_meri(cur_ele, direction) {
				...
				if (((cur_ele && cur_ele.hasClass('action-next')) || direction === 'next') || ((cur_ele && cur_ele.hasClass('action-prev')) || direction === 'prev'))  {
					if (cur_mer == "AM") {
